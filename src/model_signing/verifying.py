...
class Config:
    ...
    def verify_image(
        ...
        actual_manifest = hashing.create_manifest_from_oci_layers(
            oci_manifest,
            model_name=str(parsed_ref),
            oci_client=client,
            image_ref=ref_with_digest,
        )
    ...
    def use_elliptic_key_verifier(
        ...
        The verifier in this configuration is changed to one that performs
        verification of sigstore bundles signed by an elliptic curve private
    ...
    def use_certificate_verifier(
        ...
        *,
        ...
        The verifier in this configuration is changed to one that performs
        verification of sigstore bundles signed by a signing certificate.
        ...
        Args:
            ...
            expected_san_uris: Optional URIs that must appear in the leaf
              certificate's SubjectAltName. Binds the signature to a specific
              signer identity (e.g. a SPIFFE ID) in addition to
              chain-of-trust.
        ...
        self._verifier = certificate.Verifier(
            ...
            expected_san_uris=expected_san_uris,
        )
        return self
